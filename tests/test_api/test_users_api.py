...
@pytest.mark.asyncio
async def test_update_user_profile_success(async_client, verified_user, user_token):
    updated_data = {"nickname": "new_nickname", "bio": "Updated bio!"}
    headers = {"Authorization": f"Bearer {user_token}"}
    response = await async_client.put(f"/users/{verified_user.id}", json=updated_data, headers=headers)
    assert response.status_code == 200
...
# Professional Status Upgrade Tests
@pytest.mark.asyncio
async def test_upgrade_user_to_professional_by_admin(async_client, verified_user, admin_token):
    headers = {"Authorization": f"Bearer {admin_token}"}
    response = await async_client.put(f"/users/{verified_user.id}/upgrade", json={"status": "professional"}, headers=headers)
    assert response.status_code == 200
    assert response.json()["status"] == "professional"

@pytest.mark.asyncio
async def test_upgrade_user_to_professional_access_denied(async_client, verified_user, user_token):
    headers = {"Authorization": f"Bearer {user_token}"}
    response = await async_client.put(f"/users/{verified_user.id}/upgrade", json={"status": "professional"}, headers=headers)
    assert response.status_code == 403
